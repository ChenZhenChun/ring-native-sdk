...
- Android JitPack 聚合版本：`v1.0.7`
- iOS CocoaPods 聚合版本：`v1.0.7`
- Android 最低版本：API 26
- iOS 最低版本：iOS 12.0
- iOS `DHBleSDK`：`1.1.8`
...
```groovy
implementation "com.github.ChenZhenChun:ring-native-sdk:v1.0.7"
```
...
```ruby
pod 'OSRingSDK', :git => 'https://github.com/ChenZhenChun/ring-native-sdk.git', :tag => 'v1.0.7'
```
...
3. 本地验证 Android Maven 发布结构：
   ...
   ```bash
   VERSION=1.0.7 ./gradlew clean publishToMavenLocal --no-daemon
   ```
...
4. 提交并创建不可变标签：
   ...
   ```bash
   git add .
   git commit -m "更新戒指原生 SDK 至 1.0.7"
   git push origin master
   git tag v1.0.7
   git push origin v1.0.7
   ```

5. 在 JitPack 检查 `com.github.ChenZhenChun:ring-native-sdk:v1.0.7` 构建成功，再同步更新 `uts-ring-sdk` 两端的依赖 tag/版本。该仓库的 JitPack Maven 版本保留 Git Tag 的 `v` 前缀。
